...
def rknn_first_order_transform(data, k, transform, scale = 2000.0):
    rows_count = len(data)
    ...
    distances, R_ind= NN.radius_neighbors(data, radius = scale)
    first_order_tx = np.array(list(map(lambda p, q: np.mean(data[p] - q, axis = 0), R_ind, data)))
    rknn_values = np.sqrt(first_order_tx*first_order_tx).sum(axis=1)/scale
    ...
    #rknn_values = np.array(list(map(lambda p: np.mean(rknn_values[p]), indices)))
...
def border_peel(data, border_func, threshold_func, max_iterations = 150, min_iterations = 3, mean_border_eps = -1,
    ...
    for t in range(max_iterations):
        ...
        for d,i, nn_inds, nn_dists in zip(current_data,list(range(len(current_data))), nbrs_indices, nbrs_distances):
