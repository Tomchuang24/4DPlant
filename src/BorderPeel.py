...
class BorderPeel(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, X_plot_projection = None):
        ...
        if (self.method == "first_order_tensor"):
            border_func = lambda data: bt.rknn_first_order_transform(data, self.k, bt.exp_local_scaling_transform)
            threshold_func = lambda value: value > 0.4

        result = bt.border_peel(X, border_func, threshold_func, max_iterations=self.max_iterations,
