...
class FarthestSamplerIdx:
    ...
    def __call__(self, pts, k):
        ...
        idx[0]          = np.random.randint(len(pts))
        farthest_pts[0] = pts[idx[0]]
        distances = self._calc_distances(farthest_pts[0], pts)
        for i in range(1, k):
...
# ----------------------------------------
 #, titles
def plot_pcd_three_views(filename, pcds, suptitle='', sizes=None, cmap='Reds', zdir='y',
...
def save_pcd(filename, points, write_ascii=False):
    pcd = o3d.geometry.PointCloud()
    pcd.points = open3d.utility.Vector3dVector(points)
    o3d.io.write_point_cloud(filename, pcd, write_ascii= write_ascii)
...
def read_ply_with_color(file, count=None):
    ...
    if 'diffuse_red' in loaded['vertex'].data.dtype.names:
        colors = np.vstack(
            [loaded['vertex'].data['diffuse_red'], loaded['vertex'].data['diffuse_green'], loaded['vertex'].data['diffuse_blue']])
        if 'alpha' in loaded['vertex'].data.dtype.names:
            colors = np.concatenate([colors, np.expand_dims(loaded['vertex'].data['alpha'], axis=0)], axis=0)
        colors = colors.transpose(1, 0)
        colors = colors.astype(np.float32) / 255.0
...
def read_ply(file, count=None):
    ...
    if 'scalar_label' in loaded['vertex'].data.dtype.names:
        labels = loaded['vertex'].data['scalar_label']
        labels = np.reshape(labels, (1, len(labels)))
        points = np.concatenate([points, labels], axis=0)
        
    if 'scalar_object' in loaded['vertex'].data.dtype.names:
        labels = loaded['vertex'].data['scalar_object']
        labels = np.reshape(labels, (1, len(labels)))
        points = np.concatenate([points, labels], axis=0)
...
def save_ply_attr(points, filename, colors=None, normals=None, attr = None):
    ...
    if attr is not None:
        vertex_attr = np.core.records.fromarrays(attr.transpose(1, 0), names='attr', formats='u1')
...
def point_cloud_three_views(points, diameter=5):
    ...
    try:
        ...
        img1 = draw_point_cloud(points, zrot=0 / 180.0 * np.pi, xrot=0 / 180.0 * np.pi, yrot=0 / 180.0 * np.pi,
                                diameter=diameter)
        img2 = draw_point_cloud(points, zrot=0 / 180.0 * np.pi, xrot=90 / 180.0 * np.pi, yrot=0 / 180.0 * np.pi,
                                diameter=diameter)
        img3 = draw_point_cloud(points, zrot=0 / 180.0 * np.pi, xrot=180 / 180.0 * np.pi, yrot=0 / 180.0 * np.pi,
                                diameter=diameter)
        image_large = np.concatenate([img1, img2, img3], 1)
...
def point_cloud_three_views_demo():
    ...
    points = read_ply('C:/Users/abu.h/Dropbox (Technion Dropbox)/ISPRS Congress 2022 - Normal Estimation/code/Joelle/test/7_Yoga3.ply')
    im_array = point_cloud_three_views(points)
    img = Image.fromarray(np.uint8(im_array * 255.0))
    img.save('C:/Users/abu.h/Dropbox (Technion Dropbox)/ISPRS Congress 2022 - Normal Estimation/code/Joelle/test/yoga_J.jpg')
...
def txt2ply(fln_in, fln_out):
    pts = pd.read_csv(fln_in, delim_whitespace=True, header=None).astype(float).values
    pts_np = pts[:, 0:3]
    if np.shape(pts)[1] > 3:
        label = pts[:, 3]
    save_ply_attr(pts_np, fln_out, attr=label)
...
def get_file_path(root_path):
    ...
    Parameters
    ----------
    root_path : root path u wanna search within it

    Returns
    -------
    file_list : list of specific data
    dir_list :  list of dir
    ...
    for i in range(len(result)):
        result[i] = (result[i], os.path.getsize(result[i]))

    # Sort list by file size
    # If reverse=True sort from largest to smallest
    # If reverse=False sort from smallest to largest
    result.sort(key=lambda filename: filename[1], reverse=False)
...
if __name__ == "__main__":
    ...
    filename =r'C:/Users/abu.h/Dropbox (Technion Dropbox)/ISPRS Congress 2022 - Normal Estimation/code/Joelle/test/7_Yoga3.ply'

    load(filename, count=None)
    points_yoga = read_ply(filename, count=None)

    filename_parallel_planes =r'C:/Users/abu.h/Dropbox (Technion Dropbox)/ISPRS Congress 2022 - Normal Estimation/code/Joelle/test/parallel_planes.ply'
    points, colors = read_ply_with_color(filename_parallel_planes, count=None)
    im_array = point_cloud_three_views(points[:,:3])
    img = Image.fromarray(np.uint8(im_array * 255.0))
    img.save('C:/Users/abu.h/Dropbox (Technion Dropbox)/ISPRS Congress 2022 - Normal Estimation/code/Joelle/test/planes_J.jpg')

    # plot_pcd_three_views(filename, points1) #not enough free memory fo rimage buffer || tinker

    file_pcd =r'C:/Users/abu.h/Dropbox (Technion Dropbox)/ISPRS Congress 2022 - Normal Estimation/code/Joelle/test/7_Yoga3.pcd'
    v1 = max(points_yoga[:,0]) - min(points_yoga[:,0])
    v2 = max(points_yoga[:,1]) - min(points_yoga[:,1])
    v3 = max(points_yoga[:,2]) - min(points_yoga[:,2])
    vol = point_cloud_to_volume(points_yoga, int(max([v1,v2,v3])), radius=1.0)
    # path = r"C:\Users\Michael Zhang\Dropbox (Technion Dropbox)\Tree Project\Segmentation\MLS TREE Code and Datasets\TLS dataset\LabelledPC"
    # outpath = r"C:\Users\Michael Zhang\Dropbox (Technion Dropbox)\Tree Project\Segmentation\MLS TREE Code and Datasets\TLS dataset\LabelledPLY"
    # fln_list = get_file_path(path)
    # for fln in fln_list:
    #     outfln = outpath + "\\" + fln.split('\\')[-1].split('.')[0] + '.ply'
    #     txt2ply(fln, outfln)
