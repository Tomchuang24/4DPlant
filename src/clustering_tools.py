...
def add_random_noise(data, labels=None, noise_points_count=100):
    ...
    for v,i in zip(data[0].A1,list(range(dim))):
        min_vals[i] = v
        max_vals[i] = v

    for r in data:
        for v,i in zip(r.A1,list(range(dim))):
            if (v > max_vals[i]):
                max_vals[i] = v
            if (v < min_vals[i]):
                min_vals[i] = v
...
def draw_clusters(X, labels, colors=None, show_plt=True, show_title=False, name=None, ax=None,
    ...
    for k, i in zip(label_map, list(range(len(label_map)))):
        if k == -1:
            ...
            if use_clustes_as_keys:
                col = colors[math.floor(k)]
            else:
                print("index_{} and color_number_{}".format(i, len(colors)))
                col = colors[i]
...
def clusters_to_labels(clusters, labels, force_unique=False):
    ...
    for i,c in zip(list(range(len(new_clusters))), clusters):
        if c == -1:
            new_clusters[i] = -1
        else:
...
class MethodsEvaluation:
    ...
    def evaulate_method(self, X, labels_true, labels, name, params, show_plt=False):
        if (name not in self.scores_table):
    ...
    def cluster_params_range_evaluation(self, data, true_labels, base_params, params_range, method, method_name):
        for p in itertools.product(*list(params_range.values())):
            func_params = deepcopy(base_params)
            for i in range(len(params_range)):
                func_params[list(params_range.keys())[i]] = p[i]
    ...
    def get_evaluation_table(self):
        ...
        keys = list(self.scores_table.keys())
